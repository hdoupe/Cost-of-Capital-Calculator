...
def test_read_json_params_objects(revision_file):
    exp = {"revision": {"CIT_rate": [{"value": 0.3}]}}
...
def test_implement_reform():
    specs = Specifications()
    new_specs = {
        'profit_rate': [{"value": 0.4}],
        'm': [{"value": 0.5}],
        'start_year': [{"value": 2019}]
    }
...
def test_implement_bad_reform1():
    ...
    new_specs = {
        'profit_rate': [{"value": 1.2}]
    }

    specs.update_specifications(new_specs, raise_errors=False)

    assert len(specs.errors) > 0
    assert specs.errors == {'profit_rate': ['profit_rate 1.2 must be less than 1.0.']}
...
def test_implement_bad_reform2():
    ...
    new_specs = {
        'profit_rate': [{"value": 0.5}],
        'DeprecSystem_3yr': [{"value": 'not_a_deprec_system'}]
    }
